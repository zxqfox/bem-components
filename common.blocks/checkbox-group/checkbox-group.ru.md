...
<table>
    ...
    <tr>
        ...
        <td>Массив пар значений <code>"text=val"</code>, которые соответствуют одному чекбоксу группы. На сервер отправляется пара <code>"text=val"</code>, где текст чекбокса задается ключом <code>text</code>, а значение — ключом <code>value</code>.
            <br>Любые модификаторы блока <code>checkbox</code> могут быть переданы независимому чекбоксу из группы. Например, модификаторы <code>checked</code> или <code>disabled</code> могут быть заданы не блоку <code>checkbox-group</code>, а отдельным чекбоксам.</td>
    </tr>
</table>
